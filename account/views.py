...
from .forms import UserRegistrationsForm, UserLoginForm, EditUserForm
...
class UserRegisterView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 'your registration successfuly', 'success')
            return redirect('home:home')
        return render(request, self.template_name, {'form': form})
...
class UserLoginView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, 'you are loged', 'success')
                if self.next:
...
class UserPasswordReserView(auth_views.PasswordResetView):
...
class UserFollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            messages.error(
                request, 'you are already fllowing this user', 'danger')
        else:
...
class UserUnfollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            relation.delete()
            messages.success(request, 'you unfollwed this user', 'success')
        else:
            messages.error(
                request, 'you are not fllowing this user', 'danger')
        return redirect('account:user_profile', user.id)
